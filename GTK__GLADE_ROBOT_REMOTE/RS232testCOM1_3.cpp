...
{ cout << "Configuracoes atuais" << endl << endl;
  ...
  cout << "|--------------------------------------------|" << endl;
  cout << "| -- Comunica RS232 -- Versao 1.0 (rev01) -- |" << endl;
  cout << "|--------------------------------------------|" << endl;
  ...
  cout << "FUNCAO AINDA NAO IMPLEMENTADA!!" << endl<< endl;
...
list2.clear();
 ...
 for (i = 0 ; i <= 1 ; i++)
 {
  ...
  for (j = 0 ; j < list1.size() ; j++)
  {
      ...
      {
       cout <<  "[" << setfill('0') << setw(2) << count << "] --> " << list1[j] ;
       count++;
      }
  }
  }
...
//  return 0;
   ...
   buffer = new char[list2[op-1].size()];
   for (i = 0 ; i < list2[op-1].size()-1 ; i++ )
   buffer[i]=list2[op-1][i];
